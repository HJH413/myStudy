...
# pivotTable 함수
# values 피벗 테이블의 들어갈 값을 정한다.
# index 피벗 테이블의 row에 들어갈 값
# columns 피벗 테이블의 colums에 들어갈 값
# aggfunc 피벗 테이블의 values에 들어간 값을 index와 columns를 기준으로 정해진 옵션으로 값을 정한다.
# mean 평균값 sum 합계 min 최솟값 median 중간값 max 최댓값 count 개수 nunique 중복을 제거한 후 개수(원소 개수)
sample_pivot = concatTest.pivot_table(values='입국객수', index='국적명', columns='기준년월', aggfunc='mean')

print(sample_pivot)
print('*' * 50)

sample_pivot2 = concatTest.pivot_table(values='입국객수', index='국적명', aggfunc='max')
